<template>
  <div class="space-y-6 pb-8">
    ...
    <nav class="flex items-center space-x-2 text-sm" aria-label="Breadcrumb">
      ...
      <span :class="isDarkMode ? 'text-gray-500' : 'text-gray-400'">/</span>
      <span :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Edit Plan</span>
    </nav>
    ...
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-2xl md:text-3xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">Edit Plan</h1>
        <p class="text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">Update plan information</p>
      </div>
    </div>
    ...
    <div v-if="loading" :class="[
      ...
      <div class="flex flex-col items-center justify-center">
        ...
        <p class="mt-4" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">Loading plan information...</p>
      </div>
    </div>
...
const fetchPlan = async () => {
  try {
    ...
    const response = await axios.get(`/api/admin/plans/${planId.value}`);
    const data = response.data.data;

    form.name = data.name || '';
    form.price = data.price || '';
    form.type = data.type || '';
    form.price_id = data.price_id || '';
...
const updatePlan = async () => {
  ...
  try {
    await axios.put(`/api/admin/plans/${planId.value}`, form);
    
    showToast('Plan updated successfully', 'success');
    router.push(`/admin/plans/${planId.value}`);
  } catch (error) {
