<template>
  <div class="space-y-6 pb-8">
    ...
    <nav class="flex items-center space-x-2 text-sm" aria-label="Breadcrumb">
      ...
      <span :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Vendors Orders Analytics</span>
    </nav>

    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">Vendors Orders Analytics</h1>
        <p class="text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">Track overall platform performance and vendor analytics</p>
      </div>
      <div class="flex flex-wrap gap-2 w-full sm:w-auto">
        ...
        <select v-model="selectedVendor" @change="fetchAnalytics" 
          :class="[
    ...
    <div v-else-if="analytics" class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div :class="[
          ...
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">Active Vendors</p>
              <p class="text-2xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">{{ analytics.metrics.active_vendors }}</p>
              <p class="text-xs" :class="analytics.metrics.vendors_growth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ analytics.metrics.vendors_growth >= 0 ? '↑' : '↓' }} {{ Math.abs(analytics.metrics.vendors_growth) }}% from last period
              </p>
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div :class="[
          ...
          <h3 class="text-sm font-semibold mb-4" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Top Performing Vendors</h3>
          <div class="space-y-3">
            <div v-for="(vendor, index) in analytics.top_vendors" :key="index" class="flex items-center gap-3">
              <span class="text-sm font-medium w-6" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{ index + 1 }}</span>
              <div class="flex-1">
                <div class="flex justify-between text-sm">
                  <span class="font-medium" :class="isDarkMode ? 'text-white' : 'text-gray-900'">{{ vendor.name }}</span>
                  <span :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">${{ formatNumber(vendor.revenue) }}</span>
                </div>
                <div class="w-full rounded-full h-2 mt-1" :class="isDarkMode ? 'bg-gray-700' : 'bg-gray-200'">
                  <div class="bg-blue-600 dark:bg-blue-400 rounded-full h-2" :style="{ width: vendor.percentage + '%' }"></div>
                </div>
              </div>
            </div>
            <div v-if="analytics.top_vendors.length === 0" class="text-center py-4" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
        ...
        <div :class="[
          ...
          <h3 class="text-sm font-semibold mb-4" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Vendor Performance Summary</h3>
          <div class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody>
                <tr v-if="analytics.vendor_performance.length === 0" class="text-center">
                  <td colspan="6" class="py-4" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">No vendor data available</td>
                </tr>
                <tr v-for="vendor in analytics.vendor_performance" :key="vendor.id" class="border-b" :class="isDarkMode ? 'border-gray-700 hover:bg-gray-700/50' : 'border-gray-100 hover:bg-gray-50'">
                  <td class="py-2">
                    <router-link :to="`/admin/vendors/${vendor.id}`" class="text-blue-600 dark:text-blue-400 hover:underline">
                      {{ vendor.name }}
                    </router-link>
                  </td>
                  <td class="py-2 text-right" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">{{ vendor.total_orders }}</td>
                  <td class="py-2 text-right font-semibold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">${{ formatNumber(vendor.total_revenue) }}</td>
                  <td class="py-2 text-right" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">${{ formatNumber(vendor.average_order_value) }}</td>
                  <td class="py-2">
                    <span class="text-xs px-2 py-1 rounded-full" :class="getVendorStatusClass(vendor.status)">
                      {{ formatStatus(vendor.status) }}
                    </span>
                  </td>
                  <td class="py-2 text-sm" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{ formatDate(vendor.created_at) }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const fetchAnalytics = async () => {
  loading.value = true;
  try {
    const response = await axios.get('/api/admin/vendors/analytics', {
      params: { 
        days: dateRange.value,
        vendor_id: selectedVendor.value === 'all' ? null : selectedVendor.value
      }
    });
