...
<template>
  <div class="space-y-6 pb-8">
    ...
    <nav class="flex items-center space-x-2 text-sm" aria-label="Breadcrumb">
      ...
      <span :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">Analytics</span>
    </nav>
    ...
    <div v-if="loading" :class="[
      ...
      <div class="flex flex-col items-center justify-center">
        ...
        <p class="mt-4" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
    ...
    <div v-else-if="analytics" class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">
                ${{ formatNumber(analytics.metrics.total_revenue) }}
              </p>
              <p class="text-xs" :class="analytics.metrics.revenue_growth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ analytics.metrics.revenue_growth >= 0 ? '↑' : '↓' }} 
                {{ Math.abs(analytics.metrics.revenue_growth) }}% from last period
              </p>
            </div>
        ...
        <!-- Total Orders -->
        <div :class="[
          'rounded-2xl shadow-lg p-5 border-l-4 border-green-500 transition-colors duration-300',
          isDarkMode ? 'bg-gray-800' : 'bg-white'
        ]">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs" :class="analytics.metrics.orders_growth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ analytics.metrics.orders_growth >= 0 ? '↑' : '↓' }} 
                {{ Math.abs(analytics.metrics.orders_growth) }}% from last period
              </p>
            </div>
        ...
        <!-- Average Order Value -->
        <div :class="[
          'rounded-2xl shadow-lg p-5 border-l-4 border-purple-500 transition-colors duration-300',
          isDarkMode ? 'bg-gray-800' : 'bg-white'
        ]">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">
                ${{ formatNumber(analytics.metrics.average_order_value) }}
              </p>
              <p class="text-xs" :class="analytics.metrics.aov_growth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
                {{ analytics.metrics.aov_growth >= 0 ? '↑' : '↓' }} 
                {{ Math.abs(analytics.metrics.aov_growth) }}% from last period
              </p>
            </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div :class="[
          ...
          <h3 class="text-sm font-semibold mb-4" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
            Revenue Overview
          </h3>
          <div class="h-64">
        ...
        <div :class="[
          ...
          <div class="space-y-3">
            <div v-for="(product, index) in analytics.top_products" :key="index" class="flex items-center gap-3">
              <span class="text-sm font-medium w-6" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
                {{ index + 1 }}
              </span>
              <div class="flex-1">
                ...
                <div class="w-full rounded-full h-2 mt-1" :class="isDarkMode ? 'bg-gray-700' : 'bg-gray-200'">
                  <div 
                    class="bg-blue-600 dark:bg-blue-400 rounded-full h-2 transition-all duration-500" 
                    :style="{ width: product.percentage + '%' }"
                  ></div>
                </div>
              </div>
            </div>
            <div v-if="analytics.top_products.length === 0" 
        ...
        <div :class="[
          ...
          <div class="space-y-3">
            <div v-for="status in analytics.order_status_distribution" :key="status.status" class="flex items-center gap-3">
              <span class="text-sm font-medium w-24" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
                {{ status.label }}
              </span>
              <div class="flex-1">
                <div class="flex justify-between text-sm">
        ...
        <!-- Recent Orders -->
        <div :class="[
          'rounded-2xl shadow-lg p-4 lg:col-span-2 overflow-hidden transition-colors duration-300',
          isDarkMode ? 'bg-gray-800' : 'bg-white'
        ]">
          <h3 class="text-sm font-semibold mb-4" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
            Recent Orders
          ...
          <div class="overflow-x-auto">
            <table class="w-full">
              <thead>
                <tr class="text-left text-xs uppercase border-b" :class="[
                  isDarkMode ? 'text-gray-400 border-gray-700' : 'text-gray-500 border-gray-200'
                ]">
                  <th class="pb-2 px-2">Order #</th>
                  <th class="pb-2 px-2 hidden sm:table-cell">Customer</th>
                  <th class="pb-2 px-2 text-right">Total</th>
                  <th class="pb-2 px-2">Status</th>
                  <th class="pb-2 px-2 hidden md:table-cell">Date</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="analytics.recent_orders.length === 0" class="text-center">
                  <td colspan="5" class="py-4" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
                    No recent orders
                  </td>
                </tr>
                <tr 
                ...
                >
                  ...
                  <td class="py-2 px-2 hidden sm:table-cell" :class="isDarkMode ? 'text-gray-300' : 'text-gray-700'">
                    {{ order.customer_name }}
                  </td>
                  <td class="py-2 px-2 text-right font-semibold" :class="isDarkMode ? 'text-white' : 'text-gray-900'">
                    ${{ formatNumber(order.total_amount) }}
                  </td>
                  <td class="py-2 px-2">
                    <span class="text-xs px-2 py-1 rounded-full whitespace-nowrap" :class="getOrderStatusClass(order.status)">
                      {{ formatStatus(order.status) }}
                    </span>
                  </td>
                  <td class="py-2 px-2 text-sm hidden md:table-cell" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">
...
export default {
  setup() {
    ...
    const fetchAnalytics = async () => {
      loading.value = true;
      try {
        const response = await axios.get('/api/vendor/analytics', {
