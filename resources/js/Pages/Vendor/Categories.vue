...
const saveCategory = async () => {
  ...
  try {
    ...
    let response;
    if (isEditMode.value) {
      response = await axios.put(`/api/vendor/categories/${editId.value}`, data);
      showToast('Category updated successfully', 'success');
    } else {
      response = await axios.post('/api/vendor/categories', data);
...
const toggleStatus = async (category) => {
  try {
    await axios.put(`/api/vendor/categories/${category.id}/toggle-status`);
...
const confirmDelete = async () => {
  try {
    await axios.delete(`/api/vendor/categories/${deleteCategoryId.value}`);
...
const bulkDelete = async () => {
  ...
  if (confirm(`Are you sure you want to delete ${selectedIds.value.length} categories?`)) {
    try {
      await axios.post('/api/vendor/categories/bulk-delete', {
