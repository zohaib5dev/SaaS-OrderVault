        ...
        <table class="w-full">
          ...
          <tbody class="divide-y" :class="isDarkMode ? 'divide-gray-700' : 'divide-gray-200'">
            ...
            <tr v-for="order in orders" :key="order.id" class="transition-colors" :class="isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'">
              <td class="px-4 py-3">
                <router-link :to="`/vendor/orders/${order.id}`" class="text-sm font-medium hover:underline" :class="isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-800'">
                  {{ order.order_number }}
                </router-link>
              </td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-2">
                  ...
                  <router-link :to="`/vendor/orders/${order.id}`" class="p-1.5 rounded-lg transition-colors" :class="isDarkMode ? 'text-gray-400 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-50'" title="View Order">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const updateOrderStatus = async () => {
  ...
  try {
    const response = await axios.put(`/api/vendor/orders/${selectedOrder.value.id}`, {
...
const confirmDelete = async () => {
  ...
  deleting.value = true;
  try {
    await axios.delete(`/api/vendor/orders/${orderToDelete.value.id}`);
