<template>
  <div class="space-y-6 pb-8">
    ...
    <nav class="flex items-center space-x-2 text-sm" aria-label="Breadcrumb">
      ...
      <router-link to="/vendor/orders/all" :class="isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-500 hover:text-gray-700'">
        Orders
      </router-link>
      <span :class="isDarkMode ? 'text-gray-500' : 'text-gray-400'">/</span>
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
      <div class="flex items-center gap-3">
        <router-link to="/vendor/orders/all" class="p-2 rounded-lg transition-colors" :class="isDarkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600'" aria-label="Back to orders">
    ...
    <div v-else :class="[
      ...
      <router-link to="/vendor/orders/all" class="px-4 py-2 bg-blue-600 from-primary-500 to-primary-600 text-white rounded-lg hover:from-primary-600 hover:to-primary-700 transition-all duration-200 shadow-md inline-block">
        Return to Orders
      </router-link>
    </div>
...
const updateStatus = async () => {
  try {
    const response = await axios.put(`/api/vendor/orders/${route.params.id}/status`, {
...
const downloadOrder = async (id) => {
  try {
    const response = await axios.get(`/api/vendor/orders/${id}/download`, {
      responseType: 'blob'
    });
