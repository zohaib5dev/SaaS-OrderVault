    ...
    <div class="invoice-wrapper">
        ...
        <table style="width:100%; border-bottom:2px solid #f0f0f0; margin-bottom:30px; padding-bottom:30px;">
            <tr>
                ...
                <td style="text-align: center; width:20%; vertical-align:top;">
                    <span class="value invoice-number">{{ $invoice['invoice_number'] ?? $invoice->invoice_number ?? 'N/A' }}</span>
                </td>
        ...
        <div class="order-info-section">
            ...
            <div class="order-info-item">
                ...
                <span class="value">{{ $invoice['date'] ?? $invoice->created_at->format('F d, Y') }}</span>
            </div>
            <div class="order-info-item">
                <span class="label">Order Status</span>
                <span class="value">
                    {{ ucfirst($order['status'] ?? $invoice->status ?? 'Pending') }}
                </span>
            </div>
            <div class="order-info-item">
                <span class="label">Payment Status</span>
                <span class="value">
                    {{ ucfirst($order['payment_status'] ?? $invoice->status ?? 'Pending') }}
                </span>
            </div>
            <div class="order-info-item">
                <span class="label">Due Date</span>
                <span class="value">{{ $order['due_date'] ?? $invoice->due_date ?? ''}}</span>
            </div>
        </div>
                    ...
                    <table style="width:100%; border-collapse:collapse;">
                        <tr>
                            ...
                            <td style="text-align:right;">
                                ${{ number_format($order['subtotal'] ?? $invoice->amount ?? 0, 2) }}
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td style="border-top:2px solid #000; padding-top:10px; text-align:right;">
                                <strong>${{ number_format($order['total_amount'] ?? $invoice->amount ?? 0, 2) }}</strong>
                            </td>
                        </tr>
                    </table>
                </td>
        ...
        <div class="invoice-footer">
            ...
            <div class="admin-text">
                <strong>Generated by:</strong> {{ $company_name }}
                | <strong>Date:</strong> {{ now()->format('F d, Y h:i A') }}
                | <strong>Invoice #:</strong> {{ $invoice['invoice_number'] ?? $invoice->invoice_number ?? 'N/A' }}
            </div>
        </div>
